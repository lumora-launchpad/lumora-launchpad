...
export default function CampaignPage({
  ...
  const creator = (info?.[2]?.result as `0x${string}` | undefined) ?? ZERO;
  ...
  const progress = info?.[8]?.result ? Number(info[8].result as bigint) / 100 : 0;
  ...
  const data: CampaignDetailData = {
    ...
    currentEth: Number(formatEth(totalCommitted)),
    targetEth: Number(formatEth(targetEth)),
    progress,
    supporters,
    deadline,
      ...
      {launched ? (
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">
            The token is live. Claim your backer allocation, then trade it on the
            curve.
          </p>
          <div className="mt-4 flex flex-col items-center gap-3">
            ...
            {claimed && (
              <span className="text-sm font-semibold text-base-mint">Claimed</span>
            )}
            {token && token !== ZERO && (
              <Link href={`/token/${token}`} className="btn-ghost w-full">
                Open token page
              </Link>
            )}
          </div>
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">
            This campaign fell short. Backers can refund their full commitment.
          </p>
